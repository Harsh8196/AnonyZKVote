...
import "../interface/IAnonyZKVote.sol";

contract AnonyZKVote is IAnonyZKVote, SemaphoreCore {
    ...
    modifier onlyCoordinator(uint256 groupId , uint256 pollId) {
        require(proposals[groupId][pollId].coordinator == msg.sender, "AnonyVoting: caller is not the poll coordinator");
        _;
    }
    ...
    function createPoll(
        ...
        require(identityToCreatedDAO[_identity][groupId],"AnonyVoting:Only DAO creater make proposal");
    ...
    function addVoter(uint256 groupId,uint256 pollId, uint256 _root) public override onlyCoordinator(groupId,pollId) {
        require(proposals[groupId][pollId].state == PollState.Created, "AnonyVoting: voters can only be added before voting");
        require(!voterAdded[pollId],"AnonyVoting: voters already added.");
    ...
    function startPoll(uint256 groupId,uint256 pollId, uint256 _encryptionKey) public override onlyCoordinator(groupId,pollId) {
        require(proposals[groupId][pollId].state == PollState.Created, "AnonyVoting: poll has already been started");
    ...
    function castVote(
        ...
        require(_proposal.state == PollState.Ongoing, "AnonyVoting: vote can only be cast in an ongoing poll");
    ...
    function endPoll(uint256 groupId,uint256 pollId, uint256 decryptionKey) public override onlyCoordinator(groupId,pollId) {
        require(proposals[groupId][pollId].state == PollState.Ongoing, "AnonyVoting: poll is not ongoing");
        require(encryptionKey[groupId][pollId] == decryptionKey,"AnonyVoting: wrong encryptionKey");
    ...
    function getVote(uint256 groupId,uint256 pollId) public view override onlyAdmin(msg.sender) returns(uint256 _vote){
        require(proposals[groupId][pollId].state == PollState.Ended,"AnonyVoting: poll is not ended");
